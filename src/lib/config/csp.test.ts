...
describe('getCsp', () => {
  it('should return correct CSP for development without API URL', () => {
    ...
    expect(csp).toContain("connect-src 'self'  https://*.vercel-analytics.com https://*.vercel-insights.com http://localhost:8080 ws://localhost:*");
  });

  it('should return correct CSP for development with API URL', () => {
    const apiUrl = 'https://api.dev.com';
    const csp = getCsp(true, apiUrl);
    expect(csp).toContain(`connect-src 'self' ${apiUrl} https://*.vercel-analytics.com https://*.vercel-insights.com http://localhost:8080 ws://localhost:*`);
  });
  ...
  it('should return correct CSP for production', () => {
    ...
    expect(csp).toContain(`connect-src 'self' ${apiUrl} https://*.vercel-analytics.com https://*.vercel-insights.com`);
