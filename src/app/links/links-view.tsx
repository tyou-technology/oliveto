...
import {ArrowRight, FileText, Instagram, Mail, MapPin} from "lucide-react";
...
const LINKS: LinkItem[] = [
  {
    ...
    label: "WhatsApp",
    meta: "(43) 99123-1726",
    href: "https://wa.me/5543991231726?text=Ol%C3%A1!%20Gostaria%20de%20mais%20informa%C3%A7%C3%B5es.",
    icon: WhatsAppIcon,
    external: true,
  },
  {
  ...
  },
  {
    key: "email",
    label: "E-mail",
    meta: "contato@olivetocontabilidade.com",
    href: "mailto:contato@olivetocontabilidade.com",
    icon: Mail,
  },
  {
    key: "maps",
    label: "Como chegar",
    meta: "Atsushi Yoshii Tower",
...
const linkCardVariants = cva(
    [
      ...
      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#161616]",
    ],
    {
      variants: {
        featured: {
          true: "border-primary/40",
          false: "border-white/15",
        },
      },
...
type LinkCardBodyProps = { l: LinkItem; featured: boolean };

function LinkCardBody({l, featured}: LinkCardBodyProps) {
  const Icon = l.icon;
  return (
      <>
        <Icon className={cn(linkIconVariants({featured}))}/>
        ...
        <ArrowRight
            className="h-[18px] w-[18px] shrink-0 text-white/30 transition-all group-hover:translate-x-1 group-hover:text-black"/>
      </>
  );
}
...
export function LinksView() {
  return (
      <main
          className="relative flex min-h-screen w-full flex-col items-center bg-[#161616] px-6 py-12 font-sans text-white">
      <motion.div
        variants={container}
        initial="hidden"
        animate="show"
      ...
      >
        ...
        <motion.div variants={item} className="flex flex-col items-center">
          <Image
            ...
            height={58}
            priority
            className="h-[58px] w-[58px] mix-blend-lighten"
          />
          <Image
            src="/logo.png"
          ...
          <p className="mt-[18px] text-[10.5px] font-medium uppercase tracking-[0.32em] text-white/50">
            Perícia
            <span className="mx-[7px] align-middle text-[13px] text-primary">
              ·
            </span>
            Contabilidade
            <span className="mx-[7px] align-middle text-[13px] text-primary">
        ...
        <nav
            ...
            className="mt-9 flex w-full flex-col gap-[11px]"
        >
          {LINKS.map((l, i) => {
            const featured = i === 0;
            const cardClass = cn(linkCardVariants({featured}));
            ...
            if (l.href.startsWith("/")) {
              return (
                  <MotionLink
                      ...
                      className={cardClass}
                  >
                    <LinkCardBody l={l} featured={featured}/>
                  </MotionLink>
              );
            }
