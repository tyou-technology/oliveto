...
const articles = [
  ...
  },
  {
    ...
    category: "Política.",
    title: "Novas políticas fiscais e seus impactos na contabilidade empresarial",
...
export function ArticlesCarousel() {
  ...
  const nextSlide = useCallback(() => {
    setCurrentIndex((prev) => (prev + 1 >= articles.length - itemsPerView + 1 ? 0 : prev + 1))
  }, [])

  const prevSlide = () => {
    setCurrentIndex((prev) => (prev === 0 ? articles.length - itemsPerView : prev - 1))
  }
    ...
    <section className="py-20 bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            <span className="text-neutral-500 text-sm tracking-wider">Blog.</span>
            <h2 className="text-3xl md:text-4xl font-light text-white mt-2">
              Artigos <span className="text-[#00ff00]">recentes</span>
            </h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-3 border border-neutral-700 hover:border-[#00ff00] hover:text-[#00ff00] transition-colors"
                aria-label="Anterior"
              >
            ...
            <Link
              href="/artigos"
              className="hidden md:flex items-center gap-2 text-sm text-neutral-400 hover:text-[#00ff00] transition-colors group"
            >
              Ver todos
        ...
        >
          ...
          >
            {articles.map((article) => (
              <Link key={article.id} href={`/artigos/${article.slug}`} className="min-w-[calc(33.333%-16px)] group">
                <article className="relative">
                  ...
                  {/* Content */}
                  <span className="text-[#00ff00] text-sm font-medium">{article.category}</span>
        ...
        <div className="flex justify-center gap-2 mt-8">
          {Array.from({ length: articles.length - itemsPerView + 1 }).map((_, index) => (
            <button
              key={index}
              onClick={() => {
              ...
              className={`h-1 transition-all duration-300 ${
                index === currentIndex ? "w-8 bg-[#00ff00]" : "w-4 bg-neutral-700 hover:bg-neutral-600"
              }`}
        ...
        <div className="flex justify-center mt-8 md:hidden">
          <Link
            href="/artigos"
            className="flex items-center gap-2 text-sm border border-neutral-700 px-6 py-3 hover:border-[#00ff00] hover:text-[#00ff00] transition-colors"
          >
